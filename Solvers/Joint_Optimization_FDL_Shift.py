...
---------------------------------------------------------------------------------------------------------------------------
'''
class JointOptimizationFDLShift(torch.nn.Module):
    def __init__(self, nb_channels=3, nb_iteration=12, nb_fdl=30, rho_initial_val=1.0, device='cpu'):
        super(JointOptimizationFDLShift,self).__init__()
    ...
    def generate_views(self, fdl, render_params):
        return self.fdl_model.fdl_synthetiser_shift_views(fdl, render_params, self.synthetiser)
