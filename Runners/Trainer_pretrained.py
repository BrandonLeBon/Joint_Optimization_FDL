...
class Trainer_pretrained():
    def __init__(self, config, training_dataset, validation_dataset, model_name):
        super(Trainer_pretrained,self).__init__()
        self.config = config
    ...
    def load_solver(self):
        ...
        if self.config["solver"]["name"] == "JointOptimizationFDLShiftCoordinatesPretrained":
            from Solvers.Joint_Optimization_FDL_Shift_Coordinates_Pretrained import JointOptimizationFDLShiftCoordinatesPretrained
            return JointOptimizationFDLShiftCoordinatesPretrained(nb_channels=self.config["data"]["nb_channels"], nb_iteration=self.config["solver"]["nb_iteration"], nb_fdl=self.config["task"]["nb_fdl"], rho_initial_val=self.config["solver"]["rho"], device=self.device)
        else:
    ...
    def load_dataset(self, nb_views):
        ...
        validation_dataloader = torch.utils.data.DataLoader(dataset=validation_dataset, batch_size=int(self.config["training"]["batch_size"]), shuffle=False, drop_last=True, num_workers=8)
    ...
    def train(self):
        ...
        v = v.to(self.device)
        u = u.to(self.device)
        d_fs = torch.linspace(-1, 1, steps=nb_measurements+2)[1:-1]
        pad = (8, 8, 8, 8)
