...
class Trainer():
    ...
    def load_solver(self):
        ...
        elif self.config["solver"]["name"] == "JointOptimizationFDLShift":
            from Solvers.Joint_Optimization_FDL_Shift import JointOptimizationFDLShift
            return JointOptimizationFDLShift(nb_channels=self.config["data"]["nb_channels"], nb_iteration=self.config["solver"]["nb_iteration"], nb_fdl=self.config["task"]["nb_fdl"], rho_initial_val=self.config["solver"]["rho"], device=self.device)
        else:
